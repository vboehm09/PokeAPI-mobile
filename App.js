  ...
  return (
    <View style={[styles.loadingWrap, compact && styles.loadingWrapCompact]}>
      <Animated.Image
        ...
        style={[
          compact ? styles.loadingAnimationCompact : styles.loadingAnimation,
          { transform: [{ rotate }] },
        ]}
        resizeMode="contain"
      />
...
export default function App() {
  ...
  const getPokemonImage = (pokemonData) =>
    pokemonData.images?.[imageVariant] || pokemonData.image || null;
  ...
  const fetchPokemonBatch = async (entries, startIndex) => {
    ...
    const detailed = await Promise.all(
      chunk.map(async (entry) => {
        const endpoint =
          entry.url || `https://pokeapi.co/api/v2/pokemon/${entry.name}`;
  ...
  const fetchPokemonByGeneration = async (generationInput) => {
    ...
    try {
      ...
      const queueEntries = data.pokemon_species
        .map((s) => ({
          name: s.name,
          orderId: extractIdFromUrl(s.url),
        }))
        .sort((a, b) => a.orderId - b.orderId);
  ...
  const handleSearch = () => {
    Keyboard.dismiss();
    if (searchMode === 'type') { fetchPokemonByType(pokemonType); return; }
    if (searchMode === 'generation') { fetchPokemonByGeneration(pokemonGeneration); return; }
    if (searchMode === 'id') { fetchPokemonById(pokemonIdQuery); return; }
    setTypeInfo(null);
    clearListResults();
            ...
            >
              <View style={styles.modeRow}>
                {['name', 'type', 'generation', 'id'].map((mode) => {
                  const labels = { name: 'Por nome', type: 'Por tipo', generation: 'Por geração', id: 'Por ID' };
                  return (
                    <TouchableOpacity
                      key={mode}
                      style={[
